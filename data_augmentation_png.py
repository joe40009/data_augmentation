...
#         img = cv2.imread(img_path + a + '.png')
        img = cv2.imread(img_path)
        rotated_img = rotate_image(img,angel)
#             cv2.imwrite(rotated_imgpath + a + '_'+ str(angle) +'d.png',rotated_img[0])
...
#                 img_new = mode(args.mode, args.imgpath + a + '.png', args.xmlpath + a + '.xml')
#             else:
            img_new = mode(args.mode, args.imgpath + a + b, args.xmlpath + a + '.xml', args.max, args.min)

            if args.mode == 'h':
                img_new[0].save(args.rotated_imgpath + a + '_' + img_new[1] + b)
            ...
            if args.mode == 'b':
                ...
                print(a + '_' + img_new[1])
            if args.mode == 's':
                img_new[0].save(args.rotated_imgpath + a + '_' + 's' + str(int(img_new[0].size[0])) + str(int(img_new[0].size[1])) + b)
            ...
            if args.mode == 'a':
                ...
                image = Image.fromarray(img.astype('uint8'), 'RGB')
                image.save(args.rotated_imgpath + a + '_' + img_new[7] + b)
                print(a + '_' + img_new[7])
                tree = ET.parse(args.xmlpath + a + '.xml')
                root = tree.getroot()
                root.iter('filename')
            ...
            if args.mode == 'random':
                img_new[0].save(args.rotated_imgpath + a + '_' + img_new[5] + b)
                print(a + '_' + img_new[5])
                tree = ET.parse(args.xmlpath + a + '.xml')
                root = tree.getroot()
                root.iter('filename')
